class Task < ApplicationRecord
    ...
    def status_in_valid_list
      errors.add(:status, "must be in: #{valid_stauses.join(', ')}") unless valid_statuses.include?(self.status)
    end
    ...
    def due_at_in_future
      errors.add(:due_at, 'must be in the future.') unless self.due_at.to_utc > Time.zone.now
    end
end
